	...
	factory: (require) => {
		...
		*/
		...
		async function fetchArchivedIds() {
			...
			if (!Array.isArray(ids) || ids.some((id) => typeof id !== "string")) throw new Error("unexpected archive response");
			return ids;
		}
			...
			const runMutation = async (action, sessionId) => {
				...
				try {
					...
					setArchivedIds((current) => {
						...
						return action === "archive" ? current.includes(sessionId) ? current : [...current, sessionId] : current.filter((existing) => existing !== sessionId);
					});
				} catch (cause) {
