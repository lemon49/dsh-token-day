window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function archiveExportHeader(total, withRecord, now = /* @__PURE__ */ new Date()) {
		...
		*/
		...
		function SessionManagerSection({ useSessions, t }) {
			...
			const archivedVisible = (0, react.useMemo)(() => orderArchivedIds(ids, byId, archivedIds ?? []).filter((id) => byId[id] !== void 0), [
				ids,
				byId,
				archivedIds
			]);
			/** Archived ids that no longer have a session file (orphan registry entries). */
			const orphanCount = (0, react.useMemo)(() => Math.max(0, (archivedIds?.length ?? 0) - archivedVisible.length), [archivedIds, archivedVisible]);
			/**
			...
			const exportArchivedIds = () => {
				...
				(async () => {
					...
					try {
						...
						const withRecord = rows.filter((id) => byId[id] !== void 0).length;
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: [t("count", { count: archivedVisible.length }), orphanCount > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("em", {
										className: SessionManagerSection_module_css_default.orphanHint,
										children: t("orphanHint", { count: orphanCount })
									}) : null] }),
							...
							}), archivedIds === null || archivedVisible.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								className: SessionManagerSection_module_css_default.status,
								children: archivedIds !== null && archivedIds.length > 0 ? t("emptyArchivedOrphans") : t("emptyArchived")
							}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("ul", {
								className: SessionManagerSection_module_css_default.list,
								children: archivedVisible.map((id) => {
		...
		*/
		...
		const managerZh = {
			...
			emptyArchivedOrphans: "无有记录的归档会话（存在无记录标记，可导出清理）",
			orphanHint: "（另有 {count} 个无记录标记）",
		...
		const managerEn = {
			...
			emptyArchivedOrphans: "No archived sessions with records (orphan markers remain; export to clean up)",
			orphanHint: "(+{count} orphaned markers)",
			sessionIdLabel: "session",
			copyId: "Copy id",
			copied: "Copied",
			archive: "Archive",
