...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      notifyNoticeTitle: 'DeepSeek Harness 需要你确认',
    ...
    const en = {
      ...
      notifyNoticeTitle: 'DeepSeek Harness needs you',
    ...
    /** 需要注入的 Client 服务。 */
    const inject = ['uiSession', 'sessions', 'locale', 'slots']
    ...
    function apply(ctx) {
       ...
       * @returns {string} 标题；取不到时返回空串。
       */
      function sessionLabel(sessionId) {
        try {
          const state = ctx.sessions?.list?.getSnapshot?.()
          const title = state?.byId?.[sessionId]?.displayTitle
      ...
      function inspect() {
        try {
          ...
          for (const [sessionId, status] of snapshot) {
            ...
            const label = sessionLabel(String(sessionId))
            const what = t(KIND_KEY[pending.kind] ?? 'notifyKindUnknown')
            notified.add(key)
            show(t('notifyNoticeTitle'), label === '' ? what : `${label} · ${what}`, key)
          }
