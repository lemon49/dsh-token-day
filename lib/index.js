...
function recordEmpty(record) {
	return record.assistantRequests === 0 && record.compactionRequests === 0 && record.billedRequests === 0 && bucketsEqual(record.usage, zeroBuckets());
}
/** Apply one signed model-attributed usage sample to a cloned model table. */
function adjustModel(models, route, usage, direction, kind, billed) {
...
const tokenDayProjectionDefinition = {
	...
	apply: (state, event) => {
		...
		if (event.type === "llm/retry") {
			...
			adjustDay(days, dayKey(event.time), {
				assistant: 1,
				compaction: 0,
				billed: 0
			}, zeroBuckets(), 1);
		...
		if (event.type === "compaction/summary") {
			...
			if (event.data.usage === void 0) {
				...
				adjustModel(models, {
					provider: event.data.provider,
					model: event.data.model
				}, zeroBuckets(), 1, "compaction", false);
				adjustDay(days, day, {
					assistant: 0,
					compaction: 1,
					billed: 0
		...
		if (previous !== null) {
			...
			if (previous.billed !== billed) {
				if (previous.billed) adjustDay(days, previous.day, {
					assistant: 0,
					compaction: 0,
					billed: 1
				}, zeroBuckets(), -1);
				if (billed) adjustDay(days, day, {
					assistant: 0,
					compaction: 0,
					billed: 1
				}, zeroBuckets(), 1);
