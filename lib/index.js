 ...
 * @returns {{ exitDelayMs: number, waitForExitMs: number, killOnTimeout: boolean, hardExitMs: number }} 重启时序参数。
 */
...
/**
 * 解析重启日志的落点：优先 `DSH_HOME/cache/dsh-hot-restart`，其次系统临时目录。
...
function resolveLogPath() {
  ...
  if (typeof home === 'string' && home !== '') candidates.push(join(home, 'cache', 'dsh-hot-restart'))
  // dsh 的 shellEnv 把 DSH_HOME 注入给**工具**环境，进程自身的 env 里往往没有，
  // 所以按约定回落到 ~/.dsh，最后才是系统临时目录。
  try {
    candidates.push(join(homedir(), '.dsh', 'cache', 'dsh-hot-restart'))
  } catch {
    /* 取不到用户目录时忽略这个候选 */
  }
  candidates.push(join(tmpdir(), 'dsh-hot-restart'))
...
function scheduleRestart(ctx, options, state, port) {
  ...
  let payloadPath
  try {
    payloadPath = join(tmpdir(), `dsh-hot-restart-${process.pid}-${Date.now()}.json`)
    writeFileSync(payloadPath, JSON.stringify(payload), 'utf8')
  } catch (error) {
    state.restarting = false
    const message = error instanceof Error ? error.message : String(error)
    ctx.logger?.warn?.(`dsh-hot-restart: could not stage the relaunch payload: ${message}`)
  ...
  try {
    ...
    state.restarting = false
    const message = error instanceof Error ? error.message : String(error)
    ctx.logger?.warn?.(`dsh-hot-restart: relaunch helper failed to start: ${message}`)
    return { ok: false, error: { code: 'helper-spawn-failed', message } }
  }

  ctx.logger?.info?.(
    `dsh-hot-restart: restart scheduled (pid ${process.pid}, helper ${helper.pid ?? '?'}, exit in ${options.exitDelayMs}ms)`,
  )
  appendLog(`restart scheduled: oldPid=${process.pid} helperPid=${helper.pid ?? '?'} argv=${JSON.stringify(process.argv.slice(1))}`)
...
export function apply(ctx, config = {}) {
  ...
  ctx.inject(['webServer'], (webCtx) => {
    ...
    }), 'dsh-hot-restart: restart routes')
  })
}