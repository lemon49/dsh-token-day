/**
 * dsh-toolbox — detached relaunch helper.
 *
 ...
 *   3. 用 host 传来的 execPath + **execArgv** + argv + cwd + env **原样**拉起新进程；
 *   4. 把诊断写进日志文件，供事后排查。
 *
 * `execArgv` 不是可有可无的补充：dsh 的启动命令是
 * `node --import tsx/esm apps/cli/src/bin.ts web`，而 `--import tsx/esm` 只存在于
 * `process.execArgv`，**不在 `process.argv` 里**。漏掉它，新进程就用裸 node 跑
 * `.ts`，走 Node 的原生 type stripping —— 那是纯语法擦除、不做类型分析，于是
 * `vendor/cordis` 的 `export const enum FiberState` 不会被内联擦除，运行时链接
 * 直接失败（`does not provide an export named 'FiberState'`），新进程当场崩掉。
 * 表现就是"点了重启，服务再也没起来"。
 *
...
if (typeof payloadPath !== 'string' || payloadPath === '') {
  console.error('[dsh-toolbox] relaunch helper: missing payload path')
  process.exit(1)
}
...
try {
  ...
  console.error('[dsh-toolbox] relaunch helper: unreadable payload:', error)
  process.exit(1)
}

const { pid, execPath, argv, execArgv, cwd, env, port, logPath } = payload
if (!Number.isInteger(pid)
  || typeof execPath !== 'string' || execPath === ''
  || !Array.isArray(argv) || argv.length === 0
  || typeof cwd !== 'string' || cwd === '') {
  console.error('[dsh-toolbox] relaunch helper: malformed payload')
  process.exit(1)
}

/** Node 自身的启动参数（如 `--import tsx/esm`）；旧版 payload 没有这个字段。 */
const nodeArgs = Array.isArray(execArgv)
  ? execArgv.filter((flag) => typeof flag === 'string')
  : []
...
function log(message) {
  ...
  try {
    process.stderr.write(`[dsh-toolbox] ${line}`)
  } catch {
    /* noop */
  }
}
...
try {
  const child = spawn(execPath, [...nodeArgs, ...argv], {
    cwd,
    detached: true,
    stdio,
    windowsHide: true,
  ...
  child.unref()
  log(`new process spawned: pid=${child.pid ?? '?'} execArgv=${JSON.stringify(nodeArgs)} argv=${JSON.stringify(argv)}`)
} catch (error) {
